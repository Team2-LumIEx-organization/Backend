  ...
  try {
    ...
    if (key === 'I_AM_THE_BEST_DRIVER') {
      ...
      if (parcel.delivery_status === 'READY_TO_PICKUP_BY_DRIVER') {
        await Parcel.findByIdAndUpdate(cabinentParcel, { $set: { delivery_status: 'ON_THE_WAY' } })
        await Location.findOneAndUpdate({ location_id: Number(locationId), cabinents: { $elemMatch: { id: Number(cabinentId) } } }, {
          $set: {
      ...
      if (parcel.delivery_status === 'ON_THE_WAY') {
        await Parcel.findByIdAndUpdate(cabinentParcel, { $set: { delivery_status: 'READY_TO_PICKUP' } })
        return res.status(201).send(key)
      }
      return res.status(403).send('wrong-code');
    } else {
      ...
      if (parcel.delivery_status === 'RESERVED') {
        await Parcel.findByIdAndUpdate(cabinentParcel, { $set: { delivery_status: 'READY_TO_PICKUP_BY_DRIVER' } })
      }
      if (parcel.delivery_status === 'READY_TO_PICKUP') {
        await Parcel.findByIdAndUpdate(cabinentParcel, { $set: { delivery_status: 'DELIVERED' } })
        await Location.findOneAndUpdate({ location_id: Number(locationId), cabinents: { $elemMatch: { id: Number(cabinentId) } } }, {
          $set: {
            'cabinents.$.occupied': false,
            'cabinents.$.key': '',
